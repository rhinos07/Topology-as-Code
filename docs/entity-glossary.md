...
| `channel` | Compiled parent entity for a depth-storage lane. Carries `depth`, `entry_side`, `exit_side`, derived `flow` (`fifo`/`lifo`), optional homogeneity and the ordered list of its storage-point IDs. Each depth place references this entity through `channel` and states only its `channel_position`. |
| `section` | Exclusive subdivision of one `storage_type`. Membership is explicit: large generated ranges use `section.selector`; individual points use `section` directly or through an exception override. The compiler writes the canonical ID (`HBR.SEC_FAST`) onto every assigned storage point. |
| `storage_point` | Smallest physical/logical storage unit (formerly "storage bin"). Rack location or block location. `zone` is its logical warehouse grouping, while `position` exclusively contains physical `x`/`y`/optional `z` coordinates. It can also carry structured quantities for `size`, `max_weight` and `capacity_volume` plus `hazmat_classes` via `default_attributes`/`exceptions`. Quantities are normalized by the compiler before hashing. |
...
| `activity_area` | Functional cross-cutting grouping, orthogonal to the physical hierarchy. Unlike the single structural `section`, a `storage_point` can belong to multiple `activity_area`s simultaneously. |
