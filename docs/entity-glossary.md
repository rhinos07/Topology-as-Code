...
| `storage_point` | Smallest physical/logical storage unit (formerly "storage bin"). Rack location or block location. Can carry structured physical quantities for `size`, `max_weight` and `capacity_volume`, plus `position` and `hazmat_classes` via `default_attributes`/`exceptions` in `storage_type`. Quantities are normalized by the compiler before hashing. |
